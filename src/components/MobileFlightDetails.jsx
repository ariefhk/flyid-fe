...
export default function MobileFlightDetails({
    isMobile = true,
    data,
    isFormDataCompleted,
    HandleActionMobileFlightDetails,
    passengerType,
}) {
    const isMobileButtonRender = () => {
        if (isMobile && isFormDataCompleted) {
            return (
                <div className='fixed inset-x-0 bottom-0  flex  h-[100px] flex-col items-center justify-center gap-3  bg-white  px-5 shadow-low'>
                    <Button
                        onClick={HandleActionMobileFlightDetails}
    ...
    return (
        <>
            <h1 className=' mt-[64px] text-title-3 font-bold lg:mt-0'>Flight Details</h1>
            ...
            {/* price  */}
            <div className='invisible h-[110px] lg:hidden'></div>

            {isMobileButtonRender()}
        </>
    );
}
